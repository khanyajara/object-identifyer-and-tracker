...
def repair_video_playback(record, settings):
    ...
    record, changed = service.repair_metadata(record)
    ...
    if original_path:
        ...
        if not compressed_original or not is_playable_video_path(compressed_original):
            ...
            result = CompressionService().compress_for_playback(original_path, target)
    ...
    if processed_path and is_playable_video_path(processed_path):
        ...
        if not compressed_processed or not is_playable_video_path(compressed_processed):
            ...
            result = CompressionService().compress_for_playback(processed_path, target)
...
def main():
    ...
    ensure_cloud_sync()
    from core.capture_mode import browser_capture_enabled
    if not browser_capture_enabled(settings):
